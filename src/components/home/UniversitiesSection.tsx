      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-6">
        <div className="grid gap-10 lg:grid-cols-[.75fr_1.25fr]">
          <div>
            ...
            <Link href="/universities" className="mt-7 inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-extrabold text-slate-950 transition hover:bg-indigo-100">Browse all universities <ArrowRight className="h-4 w-4" /></Link>
          </div>

          <div className="space-y-8">
            {universities.map(([country, items]) => (
              <div key={country}>
                <div className="mb-3 flex items-center gap-2 text-sm font-extrabold text-indigo-200"><MapPin className="h-4 w-4" /> {country}</div>
                <div className="grid gap-2 sm:grid-cols-2">
                  {items.map(([name, href]: readonly [string, string]) => (
                    <Link key={href} href={href} className="group flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.045] px-4 py-3.5 text-sm font-semibold text-slate-100 transition hover:border-indigo-300/40 hover:bg-white/[0.09]">
                      <span>{name}</span><ArrowRight className="h-4 w-4 text-slate-500 transition group-hover:translate-x-1 group-hover:text-indigo-300" />
                    </Link>
                  ))}
                </div>
              </div>
