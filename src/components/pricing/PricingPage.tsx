...
const SERVICES: Record<ServiceType, { label: string; unit: string; blurb: string; icon: string }> =
  {
    editing: {
      ...
      unit: 'per 250 words',
      blurb: 'Improve your own draft with structure, clarity and citation checks.',
      icon: '✦',
    },
    tutoring: {
      label: '1:1 Tutoring',
      unit: 'per hour',
      blurb: 'Work through concepts, briefs and difficult topics with a tutor.',
      icon: '◉',
    },
    samples: {
      label: 'Worked Examples',
      unit: 'per page',
      blurb: 'Reference material to learn from — clearly not for submission.',
      icon: '▤',
    },
    examprep: {
      label: 'Exam & Viva Prep',
      unit: 'per session',
      blurb: 'Mock questions, revision planning and confidence-building coaching.',
      icon: '✓',
    },
  };
...
const TIER_LABELS: Record<string, string> = {
  ...
  technical: 'Postgraduate / technical subjects',
};
...
export default function PricingPage({ onNavigate }: PricingPageProps) {
  ...
  const price = useMemo(
    () =>
      Math.round(
        BASE_RATE[service][tier] *
          (showsTurnaround ? TURNAROUND_MULTIPLIER[turnaround] : 1) *
          units,
      ),
  ...
  const buildQuoteSummary = () =>
    `Service: ${SERVICES[service].label} | Level: ${TIER_LABELS[tier]} | ${unitLabel}${showsTurnaround ? ` | Turnaround: ${TURNAROUND_LABELS[turnaround]}` : ''} | Estimated Price: $${price.toLocaleString()}`;

  const handleLeadSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLeadError('');
    if (!leadName.trim() || leadName.trim().length < 2)
      return setLeadError('Please enter your name.');
        ...
        <div className="stack-container relative z-10 py-16 md:py-20">
          <div className="max-w-4xl">
            ...
            <h1 className="stack-hero-title">
              Simple pricing for <span>real academic support</span>
            </h1>
            <p className="stack-hero-copy">
              Choose the support you need, adjust the scope and see an estimated price instantly. No
              confusing page-count packages.
            </p>
            <div className="flex flex-wrap gap-3 mt-7 text-sm font-semibold text-white/90">
              {['Australia & UK focused', 'Upfront estimate', 'Secure booking'].map((x) => (
                <span key={x} className="stack-pill-dark">
        ...
        <div className="grid lg:grid-cols-[1.5fr_.85fr] gap-6 items-start">
          <div className="stack-card stack-pricing-panel p-6 md:p-8">
            <div className="flex items-center gap-3 mb-7">
              <div className="stack-icon-bubble">
                <Calculator className="h-5 w-5" />
              </div>
              <div>
                <p className="text-sm font-bold text-indigo-600">BUILD YOUR ESTIMATE</p>
                <h2 className="text-2xl font-bold">What support do you need?</h2>
              </div>
            </div>
            <div className="grid sm:grid-cols-2 gap-3 mb-7">
              {(Object.keys(SERVICES) as ServiceType[]).map((key) => (
                <button
                  ...
                  className={`stack-choice ${service === key ? 'is-active' : ''}`}
                >
                  <span className="text-2xl">{SERVICES[key].icon}</span>
                  <span>
                    <strong>{SERVICES[key].label}</strong>
                    <small>{SERVICES[key].blurb}</small>
                  </span>
                  {service === key && (
                    <span className="ml-auto rounded-full bg-indigo-600 text-white p-1">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <label className="stack-field">
                ...
                <select value={tier} onChange={(e) => setTier(e.target.value)}>
                  <option value="standard">
                    {TIER_LABELS.standard} (${BASE_RATE[service].standard} {SERVICES[service].unit})
                  </option>
                  <option value="technical">
                    {TIER_LABELS.technical} (${BASE_RATE[service].technical}{' '}
              ...
              <label className="stack-field md:col-span-2">
                <span>
                  {service === 'editing'
          ...
          <aside className="stack-price-card">
            <div className="relative z-10">
              <div className="flex items-center gap-2 text-indigo-200 text-sm font-bold uppercase tracking-widest">
                <Clock3 className="h-4 w-4" /> Estimated price
              </div>
              <div className="text-6xl font-black mt-3 tracking-tight">
                ${price.toLocaleString()}
              </div>
              <p className="text-white/75 mt-3">{SERVICES[service].blurb}</p>
              <button
                onClick={() => setShowCapture(true)}
                className="w-full mt-7 rounded-xl bg-white text-indigo-700 py-4 font-extrabold flex items-center justify-center gap-2 hover:translate-y-[-1px] transition"
              >
                Continue to booking <ArrowRight className="h-5 w-5" />
              </button>
              <div className="mt-7 pt-6 border-t border-white/15 space-y-3 text-sm text-white/85">
                {[
                  'Your own work remains yours to submit',
                  'Transparent estimate before booking',
                  'AU & UK support available',
                ].map((x) => (
                  <div key={x} className="flex gap-2">
                    <Check className="h-4 w-4 text-indigo-300 mt-0.5" />
                    {x}
                  </div>
                ))}
              </div>
        ...
        <div className="grid sm:grid-cols-3 gap-4 mt-6">
          {(
            [
              ...
              ['Support when needed', ShieldCheck],
            ] as const
          ).map(([label, Icon]) => (
            <div key={label} className="stack-mini-trust">
      ...
      {showCapture && (
        ...
        >
          <div className="stack-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="stack-icon-bubble mb-4">
              <ArrowRight className="h-5 w-5" />
            </div>
            <h3 className="text-2xl font-bold">Save your estimate</h3>
            <p className="text-slate-500 mt-2 mb-6">
              Enter your details and we will carry this quote into the booking form.
            </p>
            <form onSubmit={handleLeadSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full rounded-xl bg-indigo-600 text-white py-3.5 font-bold disabled:opacity-60"
              >
