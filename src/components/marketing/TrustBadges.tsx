...
const trustItems = [
  ...
  { icon: GraduationCap, label: 'Subject-Expert Tutors', color: 'text-yellow-500' },
  ...
  { icon: Lock, label: '256-bit SSL', color: 'text-indigo-500' },
  { icon: CreditCard, label: 'Secure Payment', color: 'text-green-500' },
  { icon: Award, label: 'AU Marking Rubrics', color: 'text-yellow-600' },
  { icon: MessageSquare, label: '1-on-1 Sessions', color: 'text-indigo-500' },
];
...
export default function TrustBadges({ variant = 'full' }: TrustBadgesProps) {
    ...
    return (
      <div className="flex flex-wrap justify-center gap-2">
        {trustItems.slice(0, 4).map((item, i) => (
          <div key={i} className="flex items-center gap-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-full text-xs font-medium">
            <item.icon className={`w-3.5 h-3.5 ${item.color}`} />
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-6">
      ...
      */}
      <h3 className="text-center text-sm font-semibold text-slate-500 dark:text-slate-400 mb-4 uppercase tracking-wide">
        How We Work With Australian Students
      </h3>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {trustItems.map((item, i) => (
          <div key={i} className="flex items-center gap-3 bg-white dark:bg-slate-800 rounded-lg p-3 shadow-sm border border-slate-100 dark:border-slate-700">
            <div className={`p-2 rounded-lg bg-slate-50 dark:bg-slate-700 ${item.color}`}>
