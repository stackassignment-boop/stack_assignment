  ...
  return <main className="stack-page">
    <section className="bg-slate-950 px-6 py-16 text-white md:py-20">
      <div className="mx-auto max-w-6xl">
        ...
        <h1 className="max-w-4xl text-4xl font-black tracking-tight md:text-6xl">Engineering Academic Support</h1>
        <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-300">Academic support for engineering students in Australia and the UK. Our approach is centred on helping you understand the task, improve your own work and prepare your final submission.</p>
    ...
    <section className="px-6 py-14"><div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1.2fr_.8fr]">
      <div className="stack-card p-7"><p className="text-sm font-bold uppercase tracking-wider text-indigo-600">What you can work on</p><h2 className="mt-2 text-2xl font-black">Support for real university tasks</h2><p className="mt-3 leading-7 text-slate-600 dark:text-slate-400">Common areas include technical reports, design documentation, project writing, research reviews and engineering communication. Support can focus on planning, explanation, feedback, editing or study preparation rather than producing a submission for you.</p><ul className="mt-6 grid gap-3 sm:grid-cols-2">{['Understand the assessment brief','Improve structure and academic tone','Strengthen evidence and argument','Check referencing and presentation'].map(x => <li key={x} className="flex gap-2 text-sm font-semibold"><CheckCircle2 className="h-5 w-5 shrink-0 text-emerald-500" />{x}</li>)}</ul></div>
      <div className="rounded-2xl bg-indigo-50 p-7 dark:bg-indigo-950/30"><BookOpen className="h-7 w-7 text-indigo-600" /><h2 className="mt-4 text-xl font-bold">Referencing & study resources</h2><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">Common styles for this area include IEEE / Harvard / APA 7th. Always follow your unit or university's current instructions where they differ.</p><Link href="/tools/draft-review" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-indigo-600">Open a related resource <ArrowRight className="h-4 w-4" /></Link></div>
    </div></section>
